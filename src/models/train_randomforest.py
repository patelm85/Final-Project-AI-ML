...
This script trains a Random Forest model for Walmart sales forecasting and logs
...
def train_random_forest_model(X_train, y_train):
    """Train Random Forest model with hyperparameters."""
...
def main():
    ...
    # Start MLflow run
    with mlflow.start_run(run_name=f"random-forest-{datetime.now().strftime('%Y%m%d-%H%M%S')}"):
        # Train model
        model, params = train_random_forest_model(X_train, y_train)
        ...
        artifacts_dir = project_root / 'artifacts' / 'random_forest'
        ...
        try:
            ...
            mlflow.register_model(model_uri, "RandomForest_Sales_Forecast")
            print(f"Model registered in MLflow Model Registry as: RandomForest_Sales_Forecast")
        except Exception as e:
