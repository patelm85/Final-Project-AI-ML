...
This script trains an XGBoost model for Walmart sales forecasting and logs
...
def train_xgboost_model(X_train, y_train, X_val, y_val):
    ...
    # Ensure _estimator_type is set for MLflow compatibility
...
def main():
    ...
    with mlflow.start_run(run_name=f"xgboost-{datetime.now().strftime('%Y%m%d-%H%M%S')}"):
        # Train model
        model, params = train_xgboost_model(X_train, y_train, X_val, y_val)
        ...
        artifacts_dir = project_root / 'artifacts' / 'xgboost'
        ...
        # Log model using sklearn flavor (XGBRegressor is sklearn-compatible)
        # This avoids compatibility issues with XGBoost 2.0+ and MLflow's xgboost flavor
        try:
            mlflow.sklearn.log_model(model, "model")
            print("Model successfully logged to MLflow using sklearn flavor")
            
            # Register model
            try:
                model_uri = f"runs:/{mlflow.active_run().info.run_id}/model"
                mlflow.register_model(model_uri, "XGBoost_Sales_Forecast")
                print("Model registered in MLflow Model Registry as: XGBoost_Sales_Forecast")
            except Exception as e:
                print(f"\nWarning: Failed to register model: {str(e)}")
                print("Model has been logged successfully. You can register it manually in MLflow UI.")
        except Exception as e:
            print(f"\nWarning: Failed to log model: {str(e)}")
            print("Model metrics and artifacts have been logged successfully.")
            print("You can manually log the model in MLflow UI if needed.")
